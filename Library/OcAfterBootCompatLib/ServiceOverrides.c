...
/**
  Mark SGX reserved virtual memory regions as non-runtime to reduce the amount
...
//  WhitelistSize = ((BOOT_COMPAT_CONTEXT *) Context)->Settings.MmioWhitelistSize;
  ...
  for (Index = 0; Index < NumEntries; ++Index) {
    if (Desc->NumberOfPages > 0x8000
      ...
      && (Desc->Attribute & EFI_MEMORY_WB) != 0
      && (Desc->Attribute & EFI_MEMORY_WT) == 0
      && (Desc->Attribute & EFI_MEMORY_WC) == 0
      && (Desc->Attribute & EFI_MEMORY_UC) == 0
...
//      if (!Skipped) {
        Desc->Attribute &= EFI_MEMORY_WT;
        Desc->Attribute &= EFI_MEMORY_WC;
  ...
  )
{
  ...
  if (((BOOT_COMPAT_CONTEXT *) Context)->Settings.DevirtualiseSGX) {
    DevirtualiseSGX (
      Context,
      MemoryMapSize,
      MemoryMap,
  ...
  )
{
  ...
  if (BootCompat->ServiceState.AppleBootNestedCount > 0) {
   ...
   //  if (BootCompat->Settings.DevirtualiseSGX) {
   //    DevirtualiseSGX (
   //      BootCompat,
   //      *MemoryMapSize,
   //      MemoryMap,
...
//      BootCompat->Settings.DevirtualiseSGX ? DevirtualiseSGX : NULL,
      BootCompat,
      VariableName,
      VendorGuid,
      Attributes,
